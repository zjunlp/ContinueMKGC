...
DATA_PATH = {
    # change here if you want to train other dataset
    'twitter17': {
                # 'train': 'data/twitter2017/train.txt',
                # 'dev': 'data/twitter2017/valid.txt',
                # 'test': 'data/twitter2017/test.txt',
                # 'train': 'data/twitter2017/sep-POLM-NEW/train_',
                # 'test': 'data/twitter2017/sep-POLM-NEW/test_',
                # 'dev': 'data/twitter2017/sep-POLM-NEW/valid_',
                'train': 'data/twitter2017/sep-PLOM/train_',
                'test': 'data/twitter2017/sep-PLOM/test_',
                'dev': 'data/twitter2017/sep-PLOM/valid_',
...
def main():
    ...
    parser.add_argument('--type_text', default="height", type=str,
                        help="regularization type of text. height, width, none and both are optional")
    parser.add_argument('--type_vision', default="height", type=str,
                        help="regularization type of vision. height, width, none and both are optional")
    ...
    parser.add_argument('--task_number', default=4, type=int, help="how many tasks")
    ...
    if args.do_train:
        ...
        processor = data_process(data_path, task_num=args.task_number, bert_name=args.bert_name, clip_processor=clip_processor, aux_processor=aux_processor, rcnn_processor=rcnn_processor)
        ...
        trainer = BertTrainer(train_data=train_dataloader, train_dataset=train_dataset, dev_data=dev_dataloader, test_data=test_dataloader, model=model, label_map=label_mapping, args=args, logger=logger, writer=writer)
